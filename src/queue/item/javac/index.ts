...
export function javac({ classpath, output, inputs, source }: {
  classpath: string
  output: string
  inputs: string[]
  source: string
}) {
  return FactoryExecTask('javac', ['javac', '-encoding UTF-8 -J-Dfile.encoding=UTF-8', '-source 10 -target 10', `-cp ${classpath}`, `-d ${output}`, ...inputs])
}
